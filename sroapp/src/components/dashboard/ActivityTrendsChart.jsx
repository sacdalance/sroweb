        ...
        <Card className={cn("flex flex-col h-full shadow-sm border border-gray-200", className)}>
            ...
            <CardContent className="flex-1 min-h-[180px] p-4 pt-0">
                <div className="flex h-full gap-2 mt-4">
                    {/* Y-Axis scale */}
                    <div className="flex flex-col justify-between items-end h-[120px] pr-2 text-[10px] text-gray-400 font-medium">
                        <span>{max}</span>
                        <span>{Math.round(max / 2)}</span>
                        <span>0</span>
                    </div>
                    {/* Bars */}
                    <div className="flex flex-1 h-full items-end gap-2 sm:gap-4">
                        {data.map((item) => {
                            ...
                            return (
                                <div key={item.key} className="flex-1 flex flex-col items-center group relative z-0 hover:z-20">
                                    <div className="relative w-full flex items-end justify-center h-[120px] bg-gray-50 rounded-t-sm overflow-visible border-l border-b border-gray-200">
                                        {/* Bar */}
                                        <div
                                        ...
                                        >
                                            ...
                                            {/* Tooltip on hover */}
                                            <div className="absolute bottom-full mb-5 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs py-1.5 px-3 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
